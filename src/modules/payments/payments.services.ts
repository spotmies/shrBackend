    ...
    if (file) {
        try {
            const uploadResult = await fileUploadService.uploadFile({
                file: file as any,
                bucket: 'payments', // As requested: all payments in 'payments' bucket
                folder: 'receipts'
            });
            fileUrl = uploadResult.publicUrl;
            fileId = uploadResult.id; // Including payment id generated by supabase storage
        } catch (error) {
...
// Get all payments
export const getAllThePayments = async (search?: string) => {
